...
# 자유도 선택
while True:
    dof = int(input("로봇팔 자유도 선택 (1, 2 또는 3): "))
    if dof in [1, 2, 3]:
        break

# 링크 길이 입력
l1 = float(input("첫 번째 링크 길이(cm): "))
l2 = float(input("두 번째 링크 길이(cm): ")) if dof >= 2 else 0
...
arm2 = cylinder(radius=0.12, color=color.orange) if dof >= 2 else None
arm3 = cylinder(radius=0.10, color=color.purple) if dof == 3 else None

# 목표 공
ball = sphere(pos=vector(5, 5, 5), radius=0.3, color=color.red)
...
def inverse_kinematics_3dof_xyz(x, y, z, l1, l2, l3, prev_angles):
    ...
    for phi_deg in range(-180, 181, 2):
        ...
        cost = error + 0.1 * (abs(math.degrees(theta1) - prev_angles[1]) +
                              abs(math.degrees(theta2) - prev_angles[2]) +
                              abs(math.degrees(theta3) - prev_angles[3]))
        if cost < best_cost:
            best_cost = cost
...
def update_arm():
    ...
    if dof == 1:
        ...
        reach = min(l1, math.sqrt(world_target.x**2 + world_target.y**2))  # 최대 l1까지만 확장
        arm1.pos = base
        arm1.axis = vector(math.cos(math.radians(yaw)), math.sin(math.radians(yaw)), 0) * reach
